...
CHARACTER_CREATOR_PROMPT = """你是一位擅长塑造鲜明人物形象的编剧。
...
# 人物设定规范
1. 人物命名要接地气（如"丽君"、"东升"），女频可用叠音
2. 每个人物小传100-300字
3. 必须包含：姓名、年龄、性格、背景、目标、记忆点
4. 主角必须有成长性，反派智商在线
5. 人物关系清晰，避免过于复杂

# 任务
为该剧创作人物小传。
1. 主要人物（2-4人）：主角、对手、爱情线角色
2. 重要配角（2-3人）：推动剧情的关键配角
3. 人物关系图（文字描述关系网）
    ...
    try:
        ...
        state["character_profiles"] = [
            ...
            },
            {
                ...
                "role": "反派",
                "age": "30岁",
                "personality": "嚣张跋扈，目中无人",
                "background": "富二代，仗着家族势力为非作歹",
                "goal": "打压主角，维护自己地位",
                "memory_point": "总是戴着金表炫耀"
            },
            {
                "name": "苏婉",
                "role": "爱情线",
...
async def script_writer_node(state: ScriptState, batch_end: int = None) -> ScriptState:
    ...
    for i in range(start_index, end_index):
        ...
        try:
            # 调用LLM
            llm_service = get_llm_service()
            content = await llm_service.generate_with_retry(prompt, max_tokens=2000)
            ...
            if episode_number % 5 == 0:
                print(f"[剧本生成] 第{episode_number}集完成，上下文已更新")
                print(f"  - 待回收悬念: {len(context.get('pending_hooks', []))}")
                print(f"  - 关键揭示: {len(context.get('key_revelations', []))}")
