...
def _repair_and_parse_json(response: str) -> Optional[Dict]:
    ...
    if start != -1 and end != -1 and end > start:
        ...
        # 5. 对提取的JSON尝试修复字符串中的换行符
        # 使用更安全的方式：逐字符扫描，跟踪字符串边界
        json_str = _fix_json_string_newlines(json_str)
        try:
            return json.loads(json_str)
        except json.JSONDecodeError:
            pass
...
def _extract_characters_with_bracket_matching(text: str) -> Optional[List[Dict]]:
    """
    使用括号匹配算法提取characters数组

    比正则更可靠，能正确处理嵌套结构
    """
    # 找到 "characters": 的位置
    match = re.search(r'"characters"\s*:\s*\[', text)
    if not match:
        return None
    ...
    characters_json = text[start:end]
    ...
    # 修复换行符后重试
    characters_json = _fix_json_string_newlines(characters_json)
    try:
        return json.loads(characters_json)
    except json.JSONDecodeError:
        pass
    ...
    # 修复尾随逗号后重试
    characters_json = re.sub(r',(\s*[}\]])', r'\1', characters_json)
    try:
        return json.loads(characters_json)
    except json.JSONDecodeError:
        pass
...
请只输出JSON，不要有任何其他文字。"""
    ...
    try:
        # 调用LLM（max_tokens=4000 确保完整 JSON 输出）
        llm_service = get_llm_service()
        response = await llm_service.generate_with_retry(prompt, max_tokens=4000)

        # 使用增强版JSON修复函数
        result = _repair_and_parse_json(response)
        ...
        if result and "characters" in result:
            ...
            raise ValueError("JSON解析成功但缺少characters字段")

    except Exception as e:
        logger.error(f"人物小传生成失败: {e}")
        logger.error(f"原始响应前500字: {response[:500] if 'response' in locals() else 'N/A'}")

        # 尝试使用括号匹配算法提取characters数组
        try:
            if 'response' in locals():
                characters_data = _extract_characters_with_bracket_matching(response)
                if characters_data:
                    state["character_profiles"] = characters_data
                    logger.info(f"[人物小传] 使用括号匹配提取成功，获得{len(characters_data)}个人物")
                    # 成功提取，跳过回退方案
                    raise StopIteration()
            ...
            logger.error(f"括号匹配提取失败: {extract_error}")

        # 如果前面的方法都失败，使用回退方案
        if "character_profiles" not in state or not state["character_profiles"]:
